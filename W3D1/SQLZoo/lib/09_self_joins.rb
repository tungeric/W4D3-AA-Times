...
def craiglockhart_to_sighthill
  ...
  JOIN routes AS first_leg ON start.company = first_leg.company AND start.num = first_leg.num
  JOIN stops AS transfer ON first_leg.stop_id = transfer.id
  JOIN routes AS second_leg ON transfer.id = second_leg.stop_id
  JOIN routes AS finish ON second_leg.num = finish.num and second_leg.company = finish.company
  WHERE
    START.stop_id IN (
      SELECT id
      FROM stops
